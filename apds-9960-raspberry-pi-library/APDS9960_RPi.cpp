...
{
  uint8_t val;
  printf("+++++++++ IN isGestureAvailable() IN +++++++++++++\n");
	
  /* Read value from GSTATUS register */
  if( !wireReadDataByte(APDS9960_GSTATUS, val) ) {
    printf("+++++++++ OUT isGestureAvailable() [ERROR] OUT +++++++++++++\n");
    return ERROR;
  }
  ...
  if( val == 1) {
    printf("+++++++++ OUT isGestureAvailable() [TRUE] OUT +++++++++++++\n");
    return true;
  } else {
    printf("+++++++++ OUT isGestureAvailable() [FALSE] OUT +++++++++++++\n");
    return false;
  }
  printf("+++++++++ OUT isGestureAvailable() OUT +++++++++++++\n");
}
...
int APDS9960_RPi::readGesture()
{
  ...
  int motion;
  int i;
  printf("++++++++++++ IN readGesture() IN +++++++++++++\n");
  /* Make sure that power and gesture is on and data is valid */
  if( !isGestureAvailable() || !(getMode() & 0b01000001) ) {
    printf("++++++++++++ OUT readGesture() [DIR_NONE] OUT +++++++++++++\n");
    return DIR_NONE;
  }
  printf("++++++++++++ readGesture() [gesture is available] +++++++++++++\n");
  ...
  while(1) {
    ...
    if( !wireReadDataByte(APDS9960_GSTATUS, gstatus) ) {
      printf("++++++++++++ OUT readGesture() [ERROR1] OUT +++++++++++++\n");
      return ERROR;
    }
	...
	printf("++++++++++++ OUT readGesture() [ERROR2] OUT +++++++++++++\n");
	return ERROR;
      }
	...
	bytes_read = wireReadDataBlock(  APDS9960_GFIFO_U, 
					 ...
					 (fifo_level * 4) );
	if( bytes_read == -1 ) {
	  printf("++++++++++++ OUT readGesture() [ERROR3] OUT +++++++++++++\n");
	  return ERROR;
	}
#if DEBUG
...
#endif
      resetGestureParameters();
      printf("++++++++++++ OUT readGesture()[%d] OUT +++++++++++++\n", gesture_motion_);
      return motion;
    }
  }
  printf("++++++++++++ OUT readGesture() [???] OUT +++++++++++++\n");
}
